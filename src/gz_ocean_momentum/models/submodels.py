...
velocity_scaler = TargetedTransform(ScalingTransform(10.), velocity_vars)
forcing_scaler = TargetedTransform(ScalingTransform(1e7, inverse=False),
                                   forcing_vars)

monthly_stdizer_means = SeasonalStdizer(std=False)
monthly_stdizer_means_stds = SeasonalStdizer(std=True)

transform1 = ChainedTransform((velocity_scaler, forcing_scaler,
                               monthly_stdizer_means))
